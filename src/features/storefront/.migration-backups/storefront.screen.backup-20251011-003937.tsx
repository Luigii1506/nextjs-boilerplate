...
const CustomerHeader: React.FC<CustomerHeaderProps> = ({
  ...
  // 🛒 CART CONTEXT - Ultra-fast context (compatible interface)
...
const CustomerTabNavigation: React.FC = () => {
  const { activeTab, setActiveTab, wishlist } = useStorefrontContext();
...
const TabContent: React.FC = () => {
  ...
  // 🛒 CART CONTEXT - ÚNICA FUENTE DE LA VERDAD PARA SPA (ULTRA-FAST)
    ...
    async (productId: string, quantity = 1) => {
      ...
      try {
        ...
        if (success) {
          ...
          console.error("❌ [TAB CONTENT] Ultra-fast addToCart failed");
        }
      } catch (error) {
        console.error("❌ [TAB CONTENT] Ultra-fast addToCart error:", error);
      }
    },
    [addToCart] // ✅ FIX: Include addToCart dependency to prevent stale closures
  );
