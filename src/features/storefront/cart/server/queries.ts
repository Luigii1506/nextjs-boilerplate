...
export async function getCartQuery(options: {
  ...
  try {
    // CRITICAL FIX: Prioritize userId over sessionId
    // If user is logged in, ONLY search by userId (not sessionId)
    // This prevents loading old guest carts when user is authenticated
    ...
    if (userId) {
      // User is authenticated - ONLY search by userId
      whereClause.userId = userId;
    } else if (sessionId) {
      // User is guest - search by sessionId
...
export async function clearCartQuery(options: {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      // 1. Find cart - CRITICAL FIX: Prioritize userId over sessionId
      const whereClause: any = {
        expiresAt: {
          gt: new Date(),
        },
