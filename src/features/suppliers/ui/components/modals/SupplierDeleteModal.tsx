/**
 * Native for Suppliers Module - Uses SupplierContext
 *
...
import { useDeleteSupplier } from "../../../../inventory/hooks";

/**
 * 🎯 SupplierDeleteModal Component
 */
export const SupplierDeleteModal: React.FC = () => {
  const { modals, closeDeleteModal } = useSupplierContext();
  ...
  const handleConfirmDelete = async () => {
    if (!modals.delete.data) return;

    deleteSupplier.mutate(modals.delete.data.id, {
      onSuccess: () => {
        closeDeleteModal();
      },
    });
  ...
  if (!modals.delete.isOpen || !modals.delete.data) return null;

  const deletingSupplier = modals.delete.data;
              ...
              <div className="mb-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-red-700 dark:text-red-400">
                    {deleteSupplier.error?.message || "Error desconocido"}
                  </p>
                </div>
              </div>
            )}
