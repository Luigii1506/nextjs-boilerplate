...
export const useUsersForDepartment = (department: keyof typeof DEPARTMENT_CONFIGS) => {
...
export const TestUsersProvider = ({ 
  ...
  return (
    <UsersConfigProvider value={testConfig}>
      {children}
    </UsersConfigProvider>
  );
};
...
describe('useUsers with different configurations', () => {
  ...
  it('should handle optimistic updates correctly', async () => {
    ...
    await act(async () => {
      result.current.createUser({ name: 'Test User', email: 'test@example.com' });
    });
...
export const useEnterpriseUsers = () => {
  ...
  const getEnterpriseConfig = () => {
    ...
    const roleConfig = ROLE_CONFIG_MATRIX.ROLES[user.role] || ROLE_CONFIG_MATRIX.ROLES.user;
  ...
  return (
    <div className="enterprise-dashboard">
      ...
      <div className="users-panel">
        ...
        {users.map(user => <EnterpriseUserCard key={user.id} user={user} />)}
      </div>
    </div>
  );
};
